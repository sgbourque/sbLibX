...
SB_STRUCT_BEGIN(custom_class_t, "my super hyper custom class!!", -1, "booo")
	...
	virtual void say( std::string_view msg )
	{
		...
		[[maybe_unused]] /*constexpr*/ auto decrypted2 = decrypt( test_encrypted2 );
		[[maybe_unused]] /*constexpr*/ auto decrypted_unsecured1 = decrypt( test_encrypted_unsecured1 );
		[[maybe_unused]] /*constexpr*/ std::string decrypted_unsecured2 = decrypt( test_encrypted_unsecured2 ); // converts to std::basic_string<char>

		//[[maybe_unused]] encrypted_string_view_test test_encrypted1_view = test_encrypted1;

		std::wcerr
			<< L":" << std::wstring(decrypted1) << L":\n";// << L": = :" << test_encrypted1_view << L":\n";
		std::cerr
			<< ":" << decrypted2 << ": = :" << test_encrypted2  << ":\n"
			<< ":" << decrypted_unsecured1 << ": = :" << test_encrypted_unsecured1  << ":\n"
			<< ":" << decrypted_unsecured2 << ": = :" << test_encrypted_unsecured2  << ":\n"
			<< std::endl;
...
#if COMPILATION_STRESS_MEMBERS > 0
...
#endif
#if COMPILATION_STRESS_MEMBERS > 16
...
#pragma message( "Please be patient..." )
...
#pragma message( "This might take some time..." )
...
#pragma message( "In fact, it will probably take quite a while. Go get a coffee or something..." )
...
#pragma message( "or cancel the build when you're tired..." )
...
#pragma message( "it should end within a couple of minutes, I guess..." )
...
{
	int inverse_3 = sbLibX::modular_inverse( 3 );
	std::cout << "3 * " << inverse_3 << " = " << 3 * inverse_3;
	...
	for_each( test.info_begin(), test.info_end(), [&test]( const auto& data_info )
	{
		auto key_info = test.key_info[data_info.key_index];
		std::cout
