...
#define SB_LIB_EXPORT SB_IMPORT_LIB
	...
	template< class T >
SB_LIB_EXPORT const _GUID& get_uuid( const T* );

//	template< class T >
//static inline const _GUID& get_uuid( const ref_ptr<T>& )
//{
//	return get_uuid( static_cast<const T*>(nullptr) );
//}

#ifndef SB_IID_ARGS
#define SB_IID_ARGS( X ) get_uuid( *X.ReleaseAndGetAddressOf() ), reinterpret_cast<void**>(X.ReleaseAndGetAddressOf<void>())
#endif
...
{
	InstanceConfiguration() : Configuration{ "sbDX12Config"_xhash64, "0.1"_xhash64 } {}

	xhash_string_view_t applicationName = "sbDX12"_xhash64;
